...
public class PhantomWalkNodeProcessor extends PhantomNodeProcessor {
    ...
    @Override public PathPoint getStart() {
        int ground;
    ...
    @Override public PathPoint getPathPointToCoords(double x, double y, double z) {
        return this.openPoint(MathHelper.floor(x),MathHelper.floor(y),MathHelper.floor(z));
    }

    @Override public int findPathOptions(PathPoint[] options, PathPoint curPoint, PathPoint targetPoint, float maxDist) {
        int index = 0;
        int stepHeight = 0;
        ...
        PathPoint southPoint = this.getSafePoint(curPoint.x,curPoint.y,curPoint.z+1,stepHeight,distToBlockTop,SOUTH);
        PathPoint westPoint = this.getSafePoint(curPoint.x-1,curPoint.y,curPoint.z,stepHeight,distToBlockTop,WEST);
        PathPoint eastPoint = this.getSafePoint(curPoint.x+1,curPoint.y,curPoint.z,stepHeight,distToBlockTop,EAST);
        PathPoint northPoint = this.getSafePoint(curPoint.x,curPoint.y,curPoint.z-1,stepHeight,distToBlockTop,NORTH);
        ...
        boolean openNorth = Objects.isNull(northPoint) || northPoint.nodeType==OPEN || northPoint.costMalus!=0f;
        boolean openSouth = Objects.isNull(southPoint) || southPoint.nodeType==OPEN || southPoint.costMalus!=0f;
        boolean openEast = Objects.isNull(eastPoint) || eastPoint.nodeType==OPEN || eastPoint.costMalus!=0f;
        boolean openWest = Objects.isNull(westPoint) || westPoint.nodeType==OPEN || westPoint.costMalus!=0f;
        if(openNorth && openWest) {
            PathPoint northWestPoint = this.getSafePoint(curPoint.x-1,curPoint.y,curPoint.z-1,stepHeight,
                                                         distToBlockTop,NORTH);
            index = tryAddingPoint(options,northWestPoint,index,targetPoint,maxDist);
        }
        if(openNorth && openEast) {
            PathPoint northEastPoint = this.getSafePoint(curPoint.x+1,curPoint.y,curPoint.z-1,stepHeight,
                                                         distToBlockTop,NORTH);
            index = tryAddingPoint(options,northEastPoint,index,targetPoint,maxDist);
        }
        if(openSouth && openWest) {
            PathPoint southWestPoint = this.getSafePoint(curPoint.x-1,curPoint.y,curPoint.z+1,stepHeight,
                                                         distToBlockTop,SOUTH);
    ...
    private @Nullable PathPoint getSafePoint(int x, int y, int z, int stepHeight, double distToBlockTop,
            EnumFacing facing) {
        ...
        if(type==WALKABLE) return point;
        if(Objects.isNull(point) && stepHeight> 0 && type!=FENCE && type!=TRAPDOOR) {
            point = this.getSafePoint(x,y+1,z,stepHeight-1,distToBlockTop,facing);
            if(Objects.nonNull(point) && (point.nodeType==OPEN || point.nodeType==WALKABLE) && this.entity.width<1f) {
    ...
    @Override public PathNodeType getPathNodeType(IBlockAccess cache, int x, int y, int z, EntityLiving entity,
            int xSize, int ySize, int zSize, boolean canBreakDoors, boolean canEnterDoors) {
        EnumSet<PathNodeType> enumset = EnumSet.noneOf(PathNodeType.class);
        PathNodeType type = BLOCKED;
        BlockPos pos = new BlockPos(entity);
        type = this.getPathNodeType(cache,x,y,z,xSize,ySize,zSize,canBreakDoors,canEnterDoors,enumset,type,pos);
        if(enumset.contains(FENCE)) return FENCE;
        else {
            PathNodeType type1 = BLOCKED;
            ...
            if(type==OPEN && entity.getPathPriority(type1)==0f) return OPEN;
            else return type1;
        }
    }

    public PathNodeType getPathNodeType(IBlockAccess cache, int x, int y, int z, int xSize, int ySize, int zSize,
            boolean canOpenDoorsIn, boolean canEnterDoorsIn, EnumSet<PathNodeType> set, PathNodeType type,
            BlockPos pos) {
        for(int i=0;i<xSize;i++) {
            for(int j=0;j<ySize;j++) {
                for(int k=0;k<zSize;k++) {
                    ...
                    if(type1==DOOR_WOOD_CLOSED && canOpenDoorsIn && canEnterDoorsIn)
                        type1 = WALKABLE;
                    if (type1==RAIL && !(cache.getBlockState(pos).getBlock() instanceof BlockRailBase) &&
                            !(cache.getBlockState(pos.down()).getBlock() instanceof BlockRailBase))
                        type1 = FENCE;
                    if(i==0 && j==0 && k==0) type = type1;
                    set.add(type1);
                }
            }
        }
