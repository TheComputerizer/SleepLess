...
import static mods.thecomputerizer.sleepless.registry.DataSerializerRegistry.CLASS_SERIALIZER;
...
public class PhantomEntity extends EntityMob {
    ...
    private static final DataParameter<Class<?>> CLASS_TYPE_SYNC = EntityDataManager.createKey(PhantomEntity.class,
            (DataSerializer<Class<?>>)CLASS_SERIALIZER.getSerializer());
    ...
    @SideOnly(CLIENT) public Render<?> currentRender;
    @SideOnly(CLIENT) public EntityLivingBase referenceEntity;

    public PhantomEntity(World world) {
        super(world);
        this.setHealth(this.getMaxHealth());
        this.setSize(1f,1.875f);
        this.addPotionEffect(new PotionEffect(PHASED,MAX_VALUE));
    ...
    @Override public boolean isEntityInvulnerable(DamageSource src) {
        return !src.isMagicDamage() && src!=OUT_OF_WORLD;
    }

    @Override public @Nullable IEntityLivingData onInitialSpawn(DifficultyInstance difficulty,
            @Nullable IEntityLivingData data) {
        data = super.onInitialSpawn(difficulty,data);
        if(!this.presetClass) {
            List<SpawnListEntry> spawnEntryHooks = this.world.getBiome(this.getPosition()).getSpawnableList(MONSTER);
            if(!spawnEntryHooks.isEmpty()) {
                SpawnListEntry entry = WeightedRandom.getRandomItem(this.rand,spawnEntryHooks);
    ...
    @Override protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(FOLLOW_RANGE).setBaseValue(64d);
        this.getEntityAttribute(MOVEMENT_SPEED).setBaseValue(0.25d);
        this.getEntityAttribute(ATTACK_DAMAGE).setBaseValue(4d);
        this.getEntityAttribute(MAX_HEALTH).setBaseValue(1d);
    }
    ...
    @Override protected void initEntityAI() {
        ...
        this.targetTasks.addTask(1,new PhantomNearestAttackableTarget<>(this,EntityPlayer.class,
                1,false,false,7f,this::isAggressive));
    }
    ...
    public double getJumpMotion() {
        ...
        PotionEffect jumpEffect = getActivePotionEffect(JUMP_BOOST);
    ...
    @Override public void onLivingUpdate() {
        ...
        if(this.lifespan%5==0 && this.minPlayerDistance>0f && !this.isAggressive) {
            for(EntityPlayer player : this.world.playerEntities) {
                if(player.getPosition().getDistance((int)this.posX,(int)this.posY,
                        (int)this.posZ)<=this.minPlayerDistance && CapabilityHandler.getPhantomFactor(player)>0f) {
                    this.setDead();
                    break;
                }
            }
    ...
    private boolean tryAssignShadowClass(@Nullable Class<?> potentialClass) {
        Class<? extends Entity> nextClass = Objects.nonNull(potentialClass) &&
                Entity.class.isAssignableFrom(potentialClass) ? (Class<? extends Entity>)potentialClass : null;
    ...
    private void trySizeUpdate(Class<? extends Entity> entityClass) {
        ...
        try {
            ...
            LOGGER.error("Could not update size for phantom entity using class {} as reference",entityClass,ex);
        }
        if(Objects.nonNull(entity)) setSize(entity.width,entity.height);
    }

    @Override public void writeEntityToNBT(@Nonnull NBTTagCompound tag) {
    ...
    @Override public void readEntityFromNBT(@Nonnull NBTTagCompound tag) {
        super.writeEntityToNBT(tag);
    ...
    @Override public float getBrightness() {
        ...
        return super.getBrightness()*PHANTOM_VISIBILITY;
    }
